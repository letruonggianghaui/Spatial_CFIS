...
def calU(rel_rule, img_rule, last_image_path, last_hod_path):
    ...
    for j in range(rel_rule.shape[0]):
        res=0
        alpha=5
        #A(0, a2, 0); A’(a2’, 0, 0);  C’(c2’, 0, 0); B(b2’, b2, 0);C(0, c2, 0);
...
def Adam_rel(U, rel_rule, y, w, s, learning_rate = 0.01, iterations = 150, threshold=0.0001, momentum = 0.1, beta1 = 0.9, beta2 = 0.999):
    m = y.shape[0]
    error = 0
    mt = np.zeros(3)
    vt = np.zeros(3)
...
def Adam_img(U, img_rule, y, w, s, learning_rate , iterations , threshold, momentum , beta1 , beta2, validationImage):
    m = y.shape[0]
    error = 0
    mt = np.zeros(3)
    vt = np.zeros(3)
